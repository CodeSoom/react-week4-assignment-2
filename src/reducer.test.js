...
describe('reducer', () => {
  describe('updateRestaurant', () => {
    it('입력한 레스토랑 정보를 업데이트 해준다', () => {
      const state = reducer(
        { restaurantName: '' },
        updateRestaurant('restaurantName', '시카고 피자'),
      );
  ...
  describe('addRestaurant', () => {
    function reduceAddRestaurant(restaurantName, address, category) {
      return reducer(
        {
          restaurantName,
          category,
        ...
        addRestaurant(restaurantName, category, address),
      );
    }
    context('restaurantName, category, address 모두 있을 때', () => {
      it('입력한 레스토랑을 추가해준다', () => {
        ...
        expect(state.restaurantList[0].restaurantName).toBe('김밥지옥');
